...
class project(name, author, python="python3.5", license="GPL", github=False):
    ...
    def ganerate_remote_repo(remote, remoteGitDir) :
        '''
        Generate a distant giti bare repository, then clone it locally.
        '''
        print(self.name)
        subprocess.run(["ssh", remote, "git init --bare " + remoteGitDir + project])
        target = "ssh://"+ remote + remoteGitDir + project
        subprocess.run(["git", 'clone', target])

    def ganerate_github_repo() :
        '''
        Generate a github repository, then clone it locally.
        '''
        print("Noy Implemented yet")

    def ganerate_local_repo() :
        '''
        Generate a local git non-bare repository.
        '''
        print(self.name)
#
